...
# docker build -t gladi .
# docker run -d -p 80:80 -e RAILS_MASTER_KEY=<value from config/master.key> --name gladi gladi
...
RUN apt-get update -qq && \
    apt-get install --no-install-recommends -y curl unzip libpq-dev libvips libjemalloc2 libvips sqlite3 && \
...
RUN apt-get update -qq && \
    apt-get install --no-install-recommends -y libclang-dev build-essential git libyaml-dev pkg-config && \
